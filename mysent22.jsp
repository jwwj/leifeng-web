...
<body>
	...
	<tr>
		<td>
			<div class="cantingtype">订单号：<%=mgsc.getSc_order_id() %></div>
			...
			<div class="songcandidian">菜品：<%=mgsc.getSc_note()%></div>
			...
			<div class="fabushijian1">接受者姓名:<%=mgsc.getUser_name() %></div>
			<div class="fabushijian1">接受者电话:<%=mgsc.getUser_number() %></div>
			<div class="fabushijian1">接受时间:<%=mgsc.getSc_time_start()%></div>
		</td>
		<td>

			<div class="qiangdan0">
				<form method="post" action="FinishOrderSC?sc_task_id=<%=mgsc.getSc_task_id()%>">
					<input type="submit" value="已送达" id="qiangdan0"
